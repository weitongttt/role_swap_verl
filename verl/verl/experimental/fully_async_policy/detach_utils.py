...
def assemble_batch_from_rollout_samples(
    ...
    # 保证 DataProto.concat 时所有样本的 batch key 一致，避免 tensordict KeyError
    # 尤其是有的样本有 rollout_log_probs，有的没有的情况。
    all_keys: set[str] = set(rollout_samples_batch[0].batch.keys())
    for b in rollout_samples_batch[1:]:
        all_keys.update(b.batch.keys())

    # 如果存在 rollout_log_probs，则为缺失该 key 的样本填充 0 张量
    if "rollout_log_probs" in all_keys:
        # 找一个包含 rollout_log_probs 的模板，拿到正确形状和 dtype
        template = None
        for b in rollout_samples_batch:
            if "rollout_log_probs" in b.batch.keys():
                template = b.batch["rollout_log_probs"]
                break
        if template is not None:
            for b in rollout_samples_batch:
                if "rollout_log_probs" not in b.batch.keys():
                    # 按当前样本 batch_size 复制模板形状
                    bs = b.batch.batch_size[0]
                    zeros = torch.zeros(
                        (bs,) + template.shape[1:],
                        dtype=template.dtype,
                        device=template.device,
                    )
                    b.batch["rollout_log_probs"] = zeros
