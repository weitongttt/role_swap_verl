...
class FullyAsyncTrainer(SeparateRayPPOTrainer):
    ...
    def _filter_local_samples(self, batch: DataProto) -> DataProto:
        """Filter batch to keep only samples generated by this site."""
        exchange_cfg = getattr(self.config, "exchange", None)
        if exchange_cfg and getattr(exchange_cfg, "enable_group_merge", False):
            site_id = str(getattr(exchange_cfg, "side", "?"))
            if site_id != "?" and "source_side" in batch.non_tensor_batch:
                source_sides = batch.non_tensor_batch["source_side"]
                indices = [i for i, side in enumerate(source_sides) if str(side) == site_id]
                if len(indices) > 0:
                    old_len = len(batch)
                    batch = batch[indices]
                    print(
                        f"[FullyAsyncTrainer] Filtered batch to keep local samples only. "
                        f"Kept {len(batch)}/{old_len} sequences for site={site_id}",
                        flush=True,
                    )
                else:
                    print(
                        f"[FullyAsyncTrainer] WARNING: No local samples found in batch for site={site_id}!",
                        flush=True,
                    )
        return batch
            ...
            if mask is not None:
                ...
                if masked_advs.numel() > 0:
                    ...
                    print(
                        ...
                        f"pos_frac={pos:.3f} n_seq={len(batch)}",
                        flush=True,
                    )
    ...
    def _fit_save_checkpoint(self, force=False):
        ...
        if self.config.trainer.save_freq > 0 and (
            self.current_param_version % self.config.trainer.save_freq == 0 or force or esi_close_to_expiration
        ):
