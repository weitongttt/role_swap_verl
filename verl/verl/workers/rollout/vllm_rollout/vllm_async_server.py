...
class vLLMReplica(RolloutReplica):
    ...
    async def launch_servers(self):
        ...
        for node_rank in range(nnodes):
            ...
            # Make the server actor name unique per swap run to avoid collisions across restarts.
            run_id = None
            try:
                run_id = str(getattr(self.config, "swap_role", {}).get("run_id", None))
            except Exception:
                run_id = None
            if not run_id or run_id == "None":
                run_id = os.environ.get("SWAP_RUN_ID")
            if run_id:
                name = f"{name}_{run_id}"
            # Also include swap side (A/B) to avoid collisions between side A/B rollouters.
            side = None
            try:
                side = str(getattr(self.config, "swap_role", {}).get("side", None))
            except Exception:
                side = None
            if not side or side == "None":
                side = os.environ.get("SWAP_SIDE")
