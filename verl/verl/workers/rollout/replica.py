...
class RolloutReplica(ABC):
    ...
    async def init_standalone(self):
        ...
        prefix = getattr(self.config, "pool_name_prefix", "") or ""
        resource_pool_name = (
            f"{prefix}rollout_pool_{self.replica_rank}"
            if not self.is_reward_model
            else f"{prefix}rollout_pool_reward_{self.replica_rank}"
        )
        resource_pool_spec = {
        ...
        worker_group = RayWorkerGroup(
            ...
            ray_cls_with_init=self.get_ray_class_with_init_args(),
            bin_pack=False,
            name_prefix=f"{prefix}rollout_standalone_{self.replica_rank}"
            if not self.is_reward_model
