#!/bin/bash
# run_dual_stream_A.sh
# Side A of DualStream: synchronous hybrid-engine training, cross-pool sample exchange.
#
# Flow:
#   step 1..N : rollout -> push_A -> pull_B -> train(batch_B) -> update_weights
#
...
export CUDA_VISIBLE_DEVICES=${CUDA_VISIBLE_DEVICES_A:-0}
export VLLM_USE_V1=${VLLM_USE_V1:-1}
export RAY_DEDUP_LOGS=${RAY_DEDUP_LOGS:-0}
export SWANLAB_API_KEY=${SWANLAB_API_KEY:-"hlo16D6KKxblfDAgvGxVQ"}

# ── Ray (Side A owns port 6379) ────────────────────────────────────────────
RAY_BIN="${RAY_BIN:-}"
...
RAY_TEMP_DIR_A="${RAY_TEMP_DIR_A:-/tmp/ray_ds_a}"
mkdir -p "$RAY_TEMP_DIR_A"

if ! timeout 2 bash -c "</dev/tcp/127.0.0.1/${RAY_PORT_A}" >/dev/null 2>&1; then
  CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES_A:-0}" \
    "$RAY_BIN" start --head \
      --port="${RAY_PORT_A}" \
      --num-gpus=1 \
      --num-cpus=30 \
      --temp-dir "$RAY_TEMP_DIR_A" \
      --include-dashboard=false \
      --min-worker-port=20000 \
      --max-worker-port=29999
  sleep 3
fi
export RAY_ADDRESS="127.0.0.1:${RAY_PORT_A}"

# ── exchange server coordinates ────────────────────────────────────────────
export EXCHANGE_HOST=${EXCHANGE_HOST:-127.0.0.1}
export EXCHANGE_PORT=${EXCHANGE_PORT:-18080}

# write shared run_id (B waits for this file)
...
rm -f "$EXCHANGE_RUN_ID_FILE"
rm -f "$EXCHANGE_RUN_ID_READY_FILE"
date +%s%N > "$EXCHANGE_RUN_ID_FILE"
EXCHANGE_RUN_ID="$(cat "$EXCHANGE_RUN_ID_FILE")"
echo "$EXCHANGE_RUN_ID" > "$EXCHANGE_RUN_ID_READY_FILE"
echo "[dual_stream_A] run_id=${EXCHANGE_RUN_ID}"
...
total_training_steps=200     # total trainer update steps (overrides epoch-based count)
total_epochs=1             # set large enough so dataloader never runs out
test_freq=50
save_freq=200

project_name="dual_stream"
experiment_name="ds_plan2_A"
echo "[dual_stream_A] ppo_mini_batch_size=${ppo_mini_batch_size} require_batches=${require_batches} rollout_n=${n_resp_per_prompt} required_samples=${required_samples}"
