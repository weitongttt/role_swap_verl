#!/bin/bash
# plan2 A: strict step alternation (exchange gate) + fully-async separation
set -euo pipefail
set -x
...
# Two-card single node defaults: A uses GPU0 only.
# 强制写死，避免环境变量污染
...
RAY_TEMP_DIR_A="${RAY_TEMP_DIR_A:-/tmp/ray_plan2_a}"
mkdir -p "$RAY_TEMP_DIR_A"

CPUSET_A="${CPUSET_A:-0-29}"
RAY_NUM_CPUS_A="${RAY_NUM_CPUS_A:-30}"
RAY_NUM_GPUS_A="${RAY_NUM_GPUS_A:-1}"
...
if ! timeout 2 bash -c "</dev/tcp/127.0.0.1/${RAY_PORT_A}" >/dev/null 2>&1; then
  CUDA_VISIBLE_DEVICES="$CUDA_VISIBLE_DEVICES" \
    ...
    "$RAY_BIN" start --head \
      --port="${RAY_PORT_A}" \
      --num-gpus="${RAY_NUM_GPUS_A}" \
      --num-cpus="${RAY_NUM_CPUS_A}" \
...
PY
  ...
  if [ "${GPU_COUNT:-0}" -lt 1 ]; then
    echo "[plan2_A] ERROR: existing Ray head on port ${RAY_PORT_A} reports GPU=${GPU_COUNT}." >&2
    echo "[plan2_A] Please run: ray stop --force  (will stop both A/B), then restart exchange -> A -> B." >&2
    exit 1
  fi
fi
...
# shared run_id for tcp exchange
EXCHANGE_RUN_ID_FILE="${EXCHANGE_RUN_ID_FILE:-/tmp/verl_plan2_exchange_run_id}"
rm -f "$EXCHANGE_RUN_ID_FILE" /tmp/verl_plan2_exchange_run_id.ready 2>/dev/null || true
date +%s%N > "$EXCHANGE_RUN_ID_FILE"
EXCHANGE_RUN_ID="$(cat "$EXCHANGE_RUN_ID_FILE")"
echo "[plan2_A] run_id=${EXCHANGE_RUN_ID}"
touch /tmp/verl_plan2_exchange_run_id.ready

# training hyper-params (keep conservative for first test)
